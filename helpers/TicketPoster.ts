...
export class TicketPoster {
	private readonly url = 'https://support.newtelco.de/otrs/nph-genericinterface.pl/Webservice/GenericTicketConnectorSCRIPTS/Ticket';
	private readonly user = 'scripts';
	private readonly pass = 'scripts99';

	public async postTicket(logger: ILogger, http: IHttp, subject: String, body: String): Promise<Array<TicketResult>> {
		...
		const response = await http.post(this.url, {
			...
			data: {
				Ticket: {
					...
					StateID: "1",
					PriorityID: "3",
					CustomerUser: "ndomino@newtelco.de"
				},
				Article: {
					ArticleTypeID: "8",
					SenderTypeID: "1",
					From: "Nico <ndomino@newtelco.de>",
					Subject: subject,
					Body: body,
					MimeType: "text/json",
					Charset: "UTF8"
